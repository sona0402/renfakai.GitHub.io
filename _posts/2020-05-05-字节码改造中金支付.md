...
+  更改字节码，让字节码在运行时进行替换,需要解决的问题为byte[] signature = SignatureFactory.getSigner().sign(data)进行替换，  
...
+ 如何让这一段代码在运行时候进行替换呢？在SpringBoot启动时候手动调用CpcnRedefine.getInstance().redefine();。 
