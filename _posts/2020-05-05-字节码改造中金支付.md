...
+  更改字节码，让字节码在运行时进行替换,我们要解决的问题就是将`TxBaseRequest.postProcess(Document document)  byte[] signature = SignatureFactory.getSigner().sign(data); `进行替换，
由于tips的规则，我们要怎么处理呢？不能改变方法签名 `descriptor: ()Ljava/lang/String;)  flags: ACC_PUBLIC  ->  descriptor: ()Ljava/lang/String; flags: `，就是不能改变方法的参数和权限(public,private)
